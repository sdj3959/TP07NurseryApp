...
//            var intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)
    ...
    var resultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult(), ActivityResultCallback {
        ...
        retrofitService.calendarData(dataPart, filePart).enqueue(object : Callback<String>{
            override fun onResponse(call: Call<String>, response: Response<String>) {
                //Toast.makeText(this@CalendarActivity, response.body(), Toast.LENGTH_SHORT).show()
                Snackbar.make(binding.root,response.body().toString(), Snackbar.LENGTH_SHORT).show()
            }

            override fun onFailure(call: Call<String>, t: Throwable) {
                Toast.makeText(this@CalendarActivity, "등록에 실패하였습니다. 잠시후 다시 시도 해주세요", Toast.LENGTH_SHORT).show()
            }
    ...
    fun loadData(){
        val retrofit = RetrofitHelper().getRetrofitInstance("http://dj2022.dothome.co.kr")
        val retrofitService = retrofit.create(AddRetrofitService::class.java)
        retrofitService.getCalendarData(GUserData.userData["nursery"]).enqueue(object : Callback<String>{
            override fun onResponse(call: Call<String>, response: Response<String>) {
                imgUrl = "http://dj2022.dothome.co.kr/NurseryApp/"+response.body()
                Glide.with(this@CalendarActivity).load(imgUrl).into(binding.iv)
                Snackbar.make(binding.root,"일정표를 불러왔습니다", Snackbar.LENGTH_SHORT).show()
            }

            override fun onFailure(call: Call<String>, t: Throwable) {
                Toast.makeText(this@CalendarActivity, "일정표를 불러오는데 실패하였습니다. 잠시후 다시 시도 해주세요", Toast.LENGTH_SHORT).show()
            }
        })
    }
